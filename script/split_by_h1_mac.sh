#!/usr/bin/env zsh
# split_by_h1_mac.sh — 按一级标题（# Title）切分 Markdown 文件（macOS / zsh 版）
#
# 用法：
#   zsh split_by_h1_mac.sh <输入文件路径（相对于本脚本的路径）>
#
# 示例：
...
if [[ $# -ne 1 ]]; then
    print "用法：zsh split_by_h1_mac.sh <输入文件路径（相对于本脚本）>"
    print "示例：zsh split_by_h1_mac.sh ../agents/Eigen_zh.agent.md"
    exit 1
fi
