#!/usr/bin/env bash
# fast_paste.sh — 按一级标题（# Title）切分 Markdown 文件（Bash 版）
#
# 用法：
#   bash fast_paste.sh <输入文件路径（相对于本脚本的路径）>
#
# 示例：
#   bash fast_paste.sh ../agents/Eigen_zh.agent.md
#
...
[[ $# -ne 1 ]] && {
    echo "用法：bash fast_paste.sh <输入文件路径（相对于本脚本）>"
    echo "示例：bash fast_paste.sh ../agents/Eigen_zh.agent.md"
    exit 1
}
